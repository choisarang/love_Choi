(function($){
  ...
  // 해당페이지 선택효과 =====================================
  gnbLi.eq(3).find('dt').children('a').addClass('select');
  gnbLi.eq(3).find('dd').children('a').eq(0).addClass('select');
  gnbLi.eq(3).find('dd').css({backgroundColor:"rgba(140, 177, 200, 0.1)"})
...
lastDdLink.on('blur', function(){
  ...
  // notice list 클릭시 보이기
  const nListBox = $('.list_box');
  const nListDl = nListBox.children('dl');
  const nListDt = nListDl.children('dt');
  const nDtLink = nListDt.children('a');


  nDtLink.on('click', function(e){
    e.preventDefault();
    let nDtC = $(this).hasClass('action')
